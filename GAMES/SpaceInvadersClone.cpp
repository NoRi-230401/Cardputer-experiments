...
void loop()
{
  ...
  if (M5Cardputer.Keyboard.isKeyPressed('s') && !bulletFired)
  { // Shoot
  ...
  if (bulletFired)
  {
    bulletY -= 5;
    if (bulletY < 0)
      bulletFired = false; // Reset bullet when it goes off screen
  }
  ...
  if (moveDown)
    moveDown = false; // Reset move down
